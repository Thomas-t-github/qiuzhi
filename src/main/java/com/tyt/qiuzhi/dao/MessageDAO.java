...
public interface MessageDAO {
    ...
    @Select({"select ", SELECT_FIELDS, " from ", TABLE_NAME, " where conversation_id=#{conversationId} and has_read = 0 or has_read = 1 order by id desc limit #{offset},#{limit}"})
    ...
    @Update({"update ", TABLE_NAME, " set has_read=2 where id=#{id}"})
    int updateMessageStatus(int id);

    @Update({"update ", TABLE_NAME, " set has_read=2 where conversation_id=#{conversationId}"})
    int updateMessageByConversationIdStatus(String conversationId);
